...
- `.github/workflows/deploy-on-main.yml` uses `dorny/paths-filter` to detect which of `frontend/`, `backend/`, `scraper/` changed, builds only the changed images, pushes to `ghcr.io/jeremieflahaut/pea/*`, then SSHes into the VPS (`/home/debian/projects/pea`, a git checkout). Each deploy step **self-syncs the VPS checkout first** (`git fetch origin main && git reset --hard origin/main` — `env/` is gitignored, so the prod env is preserved) so compose/Dockerfile changes reach the VPS, then `docker compose -f docker-compose.yml -f docker-compose.prod.yml pull` + `up -d --remove-orphans`. A `backend/` change builds **two** images — `php` (`--target prod`) and `nginx` — and recreates `php`, `nginx`, `scheduler`, then runs `artisan migrate --force` + `package:discover` + `config:cache`. Migrations must be safe to apply unattended.
